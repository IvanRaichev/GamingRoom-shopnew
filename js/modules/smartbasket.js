export function Basket() {
    function getBasketData() {
        const productItems = document.querySelectorAll('.smart-basket__product-item');
        const result = [];

        for (let i = 1; i < productItems.length; i++) {
            const item = productItems[i];
            const productName = item.querySelector('.smart-basket__product-name span').textContent;
            const productId = item.querySelector('.smart-basket__product-id .smart-basket__input').value;
            const productQuantity = parseInt(item.querySelector('.smart-basket__product-quantity-state').value);
            const productPriceCommon = parseInt(item.querySelector('.smart-basket__product-price-common input').value);
            const productImg = item.querySelector('.smart-basket__product-name img').src;
                ...
                (e = {
                    ...
                    showProducts: function (s) {
                        let r = t("<form>").attr({ class: "smart-basket__form", method: "POST", id: "smart-basket__form", enctype: "multipart/form-data" }),
                            ...
                            d = t("<span>").attr({ class: "smart-basket__info-msgs" }).html("Ціна за 1 товар або послугу").append(p),
                            u = t("<div>").attr({ class: "smart-basket__product-price" }).html(`Ціна / ${a.smartBasketCurrency}`).append(c).append(d);
                        c.click(() => {
                            d.fadeToggle();
                        }),
                            p.click((t) => {
                        ...
                        let m = t("<div>").attr({ class: "smart-basket__product-price-common" }).html(`Загальна ціна/${a.smartBasketCurrency}`),
                        ...
                        for (let n in s) {
                            ...
                            for (let a in s[n]) {
                                if (
                                    ...
                                    let e = t("<img>").attr({ class: "smart-basket_img", src: s[n][a], width: 60 });
                    ...
                    commonResult: function (e, s, r) {
                        let n = 0,
                            i = 0,
                            o = t("<div>").attr({ class: "smart-basket__empty-title" }).text("Кошик порожній. Ви не додали жодного товару").fadeOut(),
                            l = t("<div>").attr({ class: "smart-basket__success-title" }).text("Замовлення прийняте. Чекайте на дзвінок").css("display", "none"),
                        ...
                        if ((0 == +n ? o.fadeIn() : o.fadeOut(), r))
                            ...
                            if (!s)
                                return (
                                    ...
                                    p.html(`<span>Загальна вартість: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`),
                            ...
                            0 == +n && t("." + o.attr("class")).css("display", "block"),
                                ...
                                t("." + p.attr("class")).html(`<span>Загальна вартість: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`);
                        }
                    },
                    ...
                    userForm: function () {
                        let s = t("<div>").attr({ class: "smart-basket__user-form" }),
                            ...
                            i = t("<input>").attr({ class: "smart-basket__user-input", type: "text", placeholder: "Введіть ім'я", name: "userName", required: !0 });
                        n.append(i);
                        let f = t("<div>").attr({ class: "smart-basket__input-wrapper" }),
                            y = t("<input>").attr({ class: "smart-basket__user-input", type: "text", placeholder: "Введіть прізвище", name: "userSurname", required: !0 });
                        f.append(y);
                        let fff = t("<div>").attr({ class: "smart-basket__input-wrapper" });
                        let z = t("<div>").attr({ class: "smart-basket__input-wrapper" }),
                            x = t("<input>").attr({ class: "smart-basket__user-input", type: "text", placeholder: "Введіть адресу", name: "userAddres", required: !0 });
                        z.append(x);
                        let df = t("<div>").attr({ class: "smart-basket__input-wrapper" }),
                            dx = t("<div>").attr({ class: "country-wrapper" }),
                            dc = t("<div>").attr({ class: "country-wrapper__inner" }),
                            dv = t("<div>").attr({ class: "country-wrapper__region" }),
                            dg = t("<label>").attr({ class: "country-label", name: "country", for: "country" }),
                            dh = t("<select>").attr({ id: "country", class: "country-select" }),
                            dj = t("<option>").attr({ value: "UA" }),
                            dk = t("<option>").attr({ value: "US" }),
                            dl = t("<label>").attr({ class: "country-label", for: "region" }),
                            dz = t("<select>").attr({ id: "region", name: "region", class: "country-select" });
                        df.append(dx);
                        dx.append(dc).append(dv);
                        dc.append(dh).append(dg);
                        dh.append(dj).append(dk);
                        dv.append(dz).append(dl);

                        let zq = t("<div>").attr({ class: "exp-date_main" }),
                            q = t("<div>").attr({ class: "exp-date" }),
                            g = t("<label>").attr({ class: "exp-label", for: "month" }),
                            ww = t("<div>").attr({ class: "expiration-fields-group" }),
                            w = t("<div>").attr({ class: "month" }),
                            qw = t("<input>").attr({ class: "month-input-field", id: "month", type: "number", placeholder: "", name: "cardMounth", required: !0 }),
                            wq = t("<span>").attr({ class: "input-separator" }),
                            er = t("<div>").attr({ class: "year" }),
                            re = t("<input>").attr({ class: "month-input-field", id: "year", type: "number", placeholder: "", name: "cardYear", required: !0 }),
                            tr = t("<div>").attr({ class: "cvv-date" }),
                            tt = t("<label>").attr({ class: "cvv-label", for: "cvv" }),
                            ty = t("<input>").attr({ class: "cvv-input-field", id: "cvv", type: "number", placeholder: "", name: "cardCvv", required: !0 });
                        dj.text("Україна");
                        g.text("Термін дії");
                        tt.text("CVV");
                        dj.text("Україна");
                        dk.text("США");
                        tr.append(tt).append(ty);
                        zq.append(q).append(tr);
                        q.append(g).append(ww);
                        ww.append(w).append(wq).append(er);
                        w.append(qw);
                        er.append(re);
                        let v = t("<div>").attr({ class: "card-number additional-form" }),
                            ni = t("<div>").attr({ class: "card-number-inner" }),
                            h = t("<input>").attr({ class: "smart-basket__user-input", type: "number", placeholder: "Введіть номер картки", name: "userTest", required: !0 });
                        v.append(ni).append(zq);
                        ni.append(h);
                        let o = t("<div>").attr({ class: "smart-basket__input-wrapper" }),
                            l = t("<input>")
                                .attr({ class: "smart-basket__user-input", type: "tel", minlenght: 10, placeholder: "Введіть телефон", name: "userTel", required: a.telIsRequired })
                                .mask(a.countryCode + "(000)000-00-00");
                        o.append(l);
                        let c = t("<div>").attr({ class: "smart-basket__input-wrapper" }),
                            p = t("<input>").attr({ class: "smart-basket__user-input", type: "email", placeholder: "Введіть e-mail", name: "userEmail", required: a.emailIsRequired });
                        ...
                        if ((c.append(p), a.agreement.isRequired)) {
                            ...
                            (r = a.agreement.isLink ? t("<a>").attr({ class: "smart-basket__link-agreement", href: a.agreement.isLink, target: "_blank", rel: "nofollow" }) : t("<span>").attr({ class: "smart-basket__link-agreement" })),
                                s.is(":checked") ? r.text("Оплата при отриманні") : r.text("Онлайн оплата"),
                                e.click(function () {
                                    if (s.is(":checked")) {
                                        (r.text("Онлайн оплата"),
                                            s.val("Онлайн оплата"));
                                        $('.additional-form').toggle();
                                    }
                                    else {
                                        (r.text("Оплата при отриманні"), s.val("Оплата при отриманні"), m.attr({ disabled: !1 }), d.removeClass("smart-basket__input-wrapper_error"));
                                        $('.additional-form').toggle(false);
                                    }

                                }),
                                d.append(s).append(e).append(r);
                        }
                        let u = t("<button>").attr({ class: "smart-basket__close-form" }).text("Продовжити покупки");
                        e.closeModalBasket(u);
                        let boll;
                        let m = t("<button>").attr({ class: "smart-basket__send-form", form: "smart-basket__form", type: "submit" }).text("Зробити замовлення"),
                            k = t("<div>").attr({ class: "smart-basket__footer" });
                        r.append(n).append(f).append(z).append(df).append(o).append(c),
                            a.agreement.isChecked && r.append(d).append(fff).append(v),
                            k.append(u).append(m),
                            s.append(r).append(k),
                            m.click(function (t) {
                                t.preventDefault();
                                ...
                                if ($('.card-number').hasClass('additional-form') && $('.card-number').css('display') == 'block') {
                                    boll = true;
                                    console.log(boll);
                                }
                                else {
                                ...
                                if (a.nameIsRequired && a.telIsRequired && a.emailIsRequired) {
                                    if (boll) {
                                        if ("" === ty.val() || "" === re.val() || "" === qw.val() || "" === h.val() || "" === x.val() || "" === l.val() || "" === y.val() || "" === i.val() || "" === p.val() || r(s)) {
                                            return (
                                                "" === l.val() && l.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === i.val() && i.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === y.val() && y.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === x.val() && x.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === p.val() && p.parent().addClass("smart-basket__input-wrapper_error"),


                                                "" === h.val() && h.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === qw.val() && qw.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === re.val() && re.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === ty.val() && ty.parent().addClass("smart-basket__input-wrapper_error"),
                                                !1
                                            );
                                        }
                                    }
                                    else {
                                        if ("" === dz.val() || "" === x.val() || "" === l.val() || "" === y.val() || "" === i.val() || "" === p.val() || r(s)) {
                                            return (
                                                "" === l.val() && l.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === i.val() && i.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === y.val() && y.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === x.val() && x.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === p.val() && p.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === dz.val() && dz.parent().addClass("smart-basket__input-wrapper_error"),
                                                !1
                                            );
                                        }
                                    }
                                    ...
                                    if (boll) {
                                        if ("" === ty.val() || "" === re.val() || "" === qw.val() || "" === h.val() || "" === x.val() || "" === y.val() || "" === i.val() || !(("" !== l.val() && a.telIsRequired) || ("" !== p.val() && a.emailIsRequired)) || r(s)) {
                                            return (
                                        ...
                                        if ("" === dz.val() || "" === x.val() || "" === y.val() || "" === i.val() || !(("" !== l.val() && a.telIsRequired) || ("" !== p.val() && a.emailIsRequired)) || r(s)) {
                                            return (
                                                "" === l.val() && a.telIsRequired && l.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === p.val() && a.emailIsRequired && p.parent().addClass("smart-basket__input-wrapper_error"),
                                                "" === i.val() && i.parent().addClass("smart-basket__input-wrapper_error"),
                        ...
                        return b(dh), b(dz), b(x), b(y), b(i), b(qw), b(re), b(ty), b(h), a.telIsRequired && b(l), a.emailIsRequired && b(p), s;
                    },
                    ...
                    sendCart: function () {
                        let s = t("#smart-basket__form")[0],
                            r = new FormData(s);
                        var nameValue = $('.smart-basket__user-input:eq(0)').val();
                        var surnameValue = $('.smart-basket__user-input:eq(1)').val();
                        var addresValue = $('.smart-basket__user-input:eq(2)').val();
                        var telValue = $('.smart-basket__user-input:eq(3)').val();
                        var emailValue = $('.smart-basket__user-input:eq(4)').val();
                        var link = document.querySelector('.block').getAttribute('data-attr');
                        var productValue = getBasketData();
                        var countryValue = $('#country option:selected').val();
                        var regionValue = $('#region option:selected').val();
                        var cityValue = $('#region option:selected').text();
                        $.ajax({

                            method: "POST",
                            url: link,
                            data: { userName: nameValue, userSurname: surnameValue, userAddres: addresValue, userTel: telValue, userEmail: emailValue, products: productValue, country: countryValue, region: regionValue, city: cityValue },
                    ...
                    alertBlock: function (a) {
                        let s = t("<div>").attr({ class: "smart-basket__alert-wrapper" }),
                            ...
                            l = t("<button>").attr({ class: "smart-basket__alert-button smart-basket__alert-button_close" }).text("Продовжити покупки"),
                            c = t("<button>").attr({ class: "smart-basket__alert-button smart-basket__alert-button_by" }).text("В кошик");
                        l.click(function () {
                            t(this)
                                .parents("." + s.attr("class"))
                                .remove();
                        ...
                        return (
                            ...
                            (n.html(
                                ...
                                i.append("Товар уже додано до кошика")),
                            o.append(l).append(c),
